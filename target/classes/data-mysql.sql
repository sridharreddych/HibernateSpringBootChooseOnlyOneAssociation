...
DROP TRIGGER IF EXISTS Just_One_Of_Many ^; 
    ...
    BEFORE INSERT ON review
        FOR EACH ROW 
            BEGIN 
                IF (NEW.article_id IS NOT NULL and NEW.book_id IS  NULL and NEW.magazine_id IS  NULL) OR 
		           (NEW.article_id IS NULL  and   NEW.book_id IS NOT NULL and NEW.magazine_id IS NULL ) OR
				   (NEW.article_id IS NULL  and   NEW.book_id IS NULL and NEW.magazine_id IS NOT NULL ) OR
	               (NEW.article_id IS NOT NULL  and   NEW.book_id IS NOT NULL and NEW.magazine_id IS NOT NULL )  THEN                    
                        SIGNAL SQLSTATE '45000'
                        SET MESSAGE_TEXT='A review can be associated with only combination of two';                    
                END IF;    
            END ^;
